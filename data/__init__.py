...
def create_dataset(dataset, config, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        from data.dummy_for_debug import dataset
        train_dataset = dataset(transform_train, config['ann_root_files'], config['image_path'], split='train')
        test_dataset = dataset(transform_test, config['eval_ann_root_files'], config['eval_image_path'], split='test')
        ood_test_dataset = dataset(transform_test, config['ood_eval_ann_root_files'], config['ood_eval_image_path'], split='test')
        return train_dataset, test_dataset, ood_test_dataset
